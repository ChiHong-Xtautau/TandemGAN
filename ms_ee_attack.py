...
class MSEEAttack(MSAttack):
    ...
    def visualization_syn(self, netg_path, netge_path):
        if not self.args.dataset == 'CIFAR10':
            return

        self.set_netG()
        state_dict = torch.load(netg_path, map_location='cpu')
        self.netG.load_state_dict(state_dict)
        state_dict2 = torch.load(netge_path, map_location='cpu')
        self.netGE.load_state_dict(state_dict2)
        ...
        syn_images = self.netGE(self.netG(noise[a:a+8]))

        # print(syn_images.shape)

        self.imshow(torchvision.utils.make_grid(syn_images.detach(), nrow=4, pad_value=1))

    def visualization_adv(self, method):
        self.create_adversary(method, False)

        # get some random training images
        dataiter = iter(self.test_loader_supp)
        images, labels = dataiter.next()

        adv_images = self.perturb(images.cuda(), labels.cuda())
        # adv_images = images
        ...
        dff_imgs = (images.cuda() - adv_images) / 5.0
        images = images * 0.2 + 0.45
        adv_images = images.cuda() + dff_imgs
        # adv_images = dff_imgs

        # show images
        img = torchvision.utils.make_grid(adv_images, nrow=10, pad_value=1)

        self.imshow(img)

        # img = torchvision.utils.make_grid(images, nrow=4, pad_value=1)
        #
        # self.imshow(img)
